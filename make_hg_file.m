...
%% remove genes with exactly the same footprint
...
[~,o4] = sort([RG.chrn]);
RG = RG(o4);
% index all genes with identical positions (last of rum omitted)
identigenes = find(diff([RG.start])==0&diff([RG.end])==0&diff([RG.chrn])==0);
% eliminate same position genes with identical symbols from index
identisymbs = strcmp({RG(identigenes).symb},{RG(identigenes+1).symb});
idg2 = identigenes(~identisymbs);
% add in last of runs
idg2 = unique([idg2,idg2+1]);
% where position changed among uniquely named 
newposers = diff([RG(idg2).start])==0 & diff([RG(idg2).end])==0 & diff([RG(idg2).chrn])==0;
newposers(1) = true;

redundant_symbs = struct('symb',{RG(idg2(~newposers)).symb});
% save a record of the symbols removed
savestruct(redundant_symbs,'redundant_symbs.txt');
...
% write text file listing the genes 
...
% write text file listing the miRNAs
